...
void videoVIDS::setProperties(gem::Properties&props)
{
  ...
  for(unsigned i=0; i<keys.size(); i++) {
	...
	int format=-1;
	if (0) format=-1;
	else if ("NTSC"     == sval) format=VI_NTSC_M; /* default NTSC */
	else if ("NTSC_M"   == sval) format=VI_NTSC_M;
	else if ("PAL"      == sval) format=VI_PAL_B; /* default PAL */
	else if ("PAL_B"    == sval) format=VI_PAL_B;
	else if ("PAL_D"    == sval) format=VI_PAL_D;
	else if ("PAL_G"    == sval) format=VI_PAL_G;
	else if ("PAL_H"    == sval) format=VI_PAL_H;
	else if ("PAL_I"    == sval) format=VI_PAL_I;
	else if ("PAL_M"    == sval) format=VI_PAL_M;
	else if ("PAL_N"    == sval) format=VI_PAL_N;
	else if ("PAL_NC"   == sval) format=VI_PAL_NC;
	else if ("SECAM"    == sval) format=VI_SECAM_B; /* default SECAM */
	else if ("SECAM_B"  == sval) format=VI_SECAM_B;
	else if ("SECAM_D"  == sval) format=VI_SECAM_D;
	else if ("SECAM_G"  == sval) format=VI_SECAM_G;
	else if ("SECAM_H"  == sval) format=VI_SECAM_H;
	else if ("SECAM_K"  == sval) format=VI_SECAM_K;
	else if ("SECAM_K1" == sval) format=VI_SECAM_K1;
	else if ("SECAM_L"  == sval) format=VI_SECAM_L;
	else if ("NTSC_M_J" == sval) format=VI_NTSC_M_J;
	else if ("NTSC_433" == sval) format=VI_NTSC_433;
	if(format>=0) {
