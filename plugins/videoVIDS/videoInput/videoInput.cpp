...
void videoDevice::setSize(int w, int h)
{
  if(sizeSet) {
    if(verbose) {
      printf("SETUP: Error device size should not be set more than once \n");
    }
  } else {
    width 				= w;
    height 				= h;
...
void videoDevice::destroyGraph()
{
  ...
  while (hr == NOERROR) {
    ...
    if (FAILED(hr)) {
      if(verbose) {
        printf("SETUP: pGraph->EnumFilters() failed. \n");
      }
      return;
    }
    ...
    if (pEnum->Next(1, &pFilter, NULL) == S_OK) {
      ...
      if (FAILED(hr)) {
        if(verbose) {
          printf("SETUP: pGraph->RemoveFilter() failed. \n");
        }
        return;
      }
      if(verbose) {
...
videoDevice::~videoDevice()
{
  ...
  if( (pControl) ) {
    HR = pControl->Pause();
    if (FAILED(HR)) if(verbose) {
        printf("ERROR - Could not pause pControl\n");
      }

    HR = pControl->Stop();
    if (FAILED(HR)) if(verbose) {
        printf("ERROR - Could not stop pControl\n");
      }
  }
  ...
  if( (pVideoInputFilter) ) {
    if(verbose) {
      printf("SETUP: freeing Capture Source \n");
    }
  ...
  if( (pGrabberF) ) {
    if(verbose) {
      printf("SETUP: freeing Grabber Filter  \n");
    }
    (pGrabberF)->Release();
    (pGrabberF) = 0;
  }
  ...
  if( (pAmMediaType) ) {
    if(verbose) {
      printf("SETUP: freeing Media Type  \n");
    }
  ...
  if( (pCaptureGraph) ) {
    if(verbose) {
      printf("SETUP: freeing Capture Graph \n");
    }
  ...
  if( (pGraph) ) {
    if(verbose) {
      printf("SETUP: freeing Main Graph \n");
    }
    (pGraph)->Release();
    (pGraph) = 0;
  }
...
void makeGUID( GUID *guid, unsigned long Data1, unsigned short Data2,
...
videoInput::videoInput()
{
  ...
  makeGUID( &MEDIASUBTYPE_Y8, 0x20203859, 0x0000, 0x0010, 0x80, 0x00, 0x00,
            0xAA, 0x00, 0x38, 0x9B, 0x71 );
  makeGUID( &MEDIASUBTYPE_GREY, 0x59455247, 0x0000, 0x0010, 0x80, 0x00, 0x00,
            0xAA, 0x00, 0x38, 0x9B, 0x71 );
...
void videoInput::setAutoReconnectOnFreeze(int deviceNumber,
    bool doReconnect, int numMissedFramesBeforeReconnect)
{
  if(deviceNumber >= VI_MAX_CAMERAS) {
    return;
  }
...
bool videoInput::setFormat(int deviceNumber, int format)
{
  ...
  if(format >= 0 && format < VI_NUM_FORMATS) {
    ...
    if(VDList[deviceNumber]->specificFormat) {
      ...
      HRESULT hr = getDevice(&VDList[deviceNumber]->pVideoInputFilter,
                             deviceNumber, VDList[deviceNumber]->wDeviceName,
      ...
      hr = VDList[deviceNumber]->pCaptureGraph->FindInterface(NULL,
           &MEDIATYPE_Video, VDList[deviceNumber]->pVideoInputFilter,
...
bool videoInput::getPixels(int id, unsigned char * dstBuffer,
                           bool flipRedAndBlue, bool flipImage)
{
  ...
  if(isDeviceSetup(id)) {
    if(bCallback) {
      ...
      HRESULT hr = VDList[id]->pGrabber->GetCurrentBuffer(&bufferSize,
                   (long *)VDList[id]->pBuffer);
      if(hr==S_OK) {
        ...
        if(verbose) {
          printf("ERROR: GetPixels() - Unable to grab frame for device %i\n", id);
        }
      }
    }
  }
...
// Returns a buffer
// ----------------------------------------------------------------------
unsigned char * videoInput::getPixels(int id, bool flipRedAndBlue,
                                      bool flipImage)
{
...
bool videoInput::isFrameNew(int id)
{
  ...
  if(VDList[id]->nFramesRunning > 400
      && VDList[id]->sgCallback->freezeCheck >
      VDList[id]->nFramesForReconnect ) {
    freeze = true;
  }
  ...
  if(freeze && VDList[id]->autoReconnect) {
    if(verbose) {
      printf("ERROR: Device seems frozen - attempting to reconnect\n");
    }
    if( !restartDevice(VDList[id]->myID) ) {
      ...
      if(verbose) {
        printf("SUCCESS: Able to reconnect to device\n");
      }
    }
  }
...
void videoInput::showSettingsWindow(int id)
{
  ...
  if(isDeviceSetup(id)) {
    ...
    HRESULT hr = getDevice(&VDList[id]->pVideoInputFilter, id,
                           VDList[id]->wDeviceName, VDList[id]->nDeviceName);
    if(hr == S_OK) {
...
bool videoInput::getVideoSettingFilter(int deviceID, long Property,
                                       long &min, long &max, long &SteppingDelta, long &currentValue, long &flags,
                                       long &defaultValue)
{
  ...
  hr = VD->pVideoInputFilter->QueryInterface(IID_IAMVideoProcAmp,
       (void**)&pAMVideoProcAmp);
  if(FAILED(hr)) {
  ...
  if (verbose) {
    printf("Setting video setting %ld.\n", Property);
  }

  pAMVideoProcAmp->GetRange(Property, &min, &max, &SteppingDelta,
                            &defaultValue, &flags);
  if (verbose) {
...
// Set a video signal setting using IAMVideoProcAmp
bool videoInput::setVideoSettingFilterPct(int deviceID, long Property,
    float pctValue, long Flags)
{
  if( !isDeviceSetup(deviceID) ) {
    return false;
  }
  ...
  if( !getVideoSettingFilter(deviceID, Property, min, max, stepAmnt,
                             currentValue, flags, defaultValue) ) {
    return false;
  }
...
// Set a video signal setting using IAMVideoProcAmp
bool videoInput::setVideoSettingFilter(int deviceID, long Property,
                                       long lValue, long Flags, bool useDefaultValue)
{
  if( !isDeviceSetup(deviceID) ) {
    return false;
  }
  ...
  long CurrVal, Min, Max, SteppingDelta, Default, CapsFlags,
       AvailableCapsFlags = 0;


  pAMVideoProcAmp->GetRange(Property, &Min, &Max, &SteppingDelta, &Default,
                            &AvailableCapsFlags);
  if (verbose) {
    printf("Range for video setting %ld: Min:%ld Max:%ld SteppingDelta:%ld Default:%ld Flags:%ld\n",
  ...
  if (verbose) {
    printf("Current value: %ld Flags %ld (%s)\n", CurrVal, CapsFlags,
...
bool videoInput::setVideoSettingCameraPct(int deviceID, long Property,
    float pctValue, long Flags)
{
  if( !isDeviceSetup(deviceID) ) {
    return false;
  ...
  if( !getVideoSettingCamera(deviceID, Property, min, max, stepAmnt,
                             currentValue, flags, defaultValue) ) {
    return false;
  }
  ...
  return setVideoSettingCamera(deviceID, Property, rasterValue, Flags,
                               false);
}


bool videoInput::setVideoSettingCamera(int deviceID, long Property,
                                       long lValue, long Flags, bool useDefaultValue)
{
  IAMCameraControl *pIAMCameraControl;
  if(isDeviceSetup(deviceID)) {
    HRESULT hr;
    hr = getDevice(&VDList[deviceID]->pVideoInputFilter, deviceID,
                   VDList[deviceID]->wDeviceName, VDList[deviceID]->nDeviceName);

    if (verbose) {
      printf("Setting video setting %ld.\n", Property);
    }
    hr = VDList[deviceID]->pVideoInputFilter->QueryInterface(
           IID_IAMCameraControl, (void**)&pIAMCameraControl);
    if (FAILED(hr)) {
      printf("Error\n");
      return false;
    } else {
      long CurrVal, Min, Max, SteppingDelta, Default, CapsFlags,
           AvailableCapsFlags;
      pIAMCameraControl->GetRange(Property, &Min, &Max, &SteppingDelta, &Default,
                                  &AvailableCapsFlags);
      if (verbose) {
        printf("Range for video setting %ld: Min:%ld Max:%ld SteppingDelta:%ld Default:%ld Flags:%ld\n",
               Property, Min, Max, SteppingDelta, Default, AvailableCapsFlags);
...
bool videoInput::getVideoSettingCamera(int deviceID, long Property,
                                       long &min, long &max, long &SteppingDelta, long &currentValue, long &flags,
                                       long &defaultValue)
{
  if( !isDeviceSetup(deviceID) ) {
  ...
  hr = VD->pVideoInputFilter->QueryInterface(IID_IAMCameraControl,
       (void**)&pIAMCameraControl);
  if(FAILED(hr)) {
    printf("setVideoSetting - QueryInterface Error\n");
    if(VD->pVideoInputFilter) {
      VD->pVideoInputFilter->Release();
  ...
  pIAMCameraControl->GetRange(Property, &min, &max, &SteppingDelta,
                              &defaultValue, &flags);
  if (verbose) {
    printf("Range for video setting %ld: Min:%ld Max:%ld SteppingDelta:%ld Default:%ld Flags:%ld\n",
           Property, min, max, SteppingDelta, defaultValue, flags);
...
{
  if(comInitCount > 0) {
    comInitCount--;  //decrease the count of instances using com
  }
...
bool videoInput::setup(int deviceNumber)
{
  ...
  if(deviceNumber>devicesFound-1) {
    if(verbose) {
      printf("SETUP: device[%i] not found - you have %i devices available\n",
             deviceNumber, devicesFound);
    }
    if(devicesFound>=0) if(verbose) {
        printf("SETUP: this means that the last device you can use is device[%i] \n",
               devicesFound-1);
      }
    return false;
  }
  ...
  if(VDList[deviceNumber]->readyToCapture) {
    if(verbose) {
      printf("SETUP: can't setup, device %i is currently being used\n",
...
void videoInput::processPixels(unsigned char * src, unsigned char * dst,
                               int width, int height, bool bRGB, bool bFlip)
{
...
//-------------------------------------------------------------------------------------------
static void findClosestSizeAndSubtype(videoDevice * VD, int widthIn,
                                      int heightIn, int &widthOut, int &heightOut, GUID & mediatypeOut)
{
  HRESULT hr;
  ...
  if (iSize == sizeof(VIDEO_STREAM_CONFIG_CAPS)) {
    ...
    for (int iFormat = 0; iFormat < iCount; iFormat++) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        else if( abs(widthIn - tempW) + abs(heightIn - tempH)  < abs(
                   widthIn - nearW) + abs(heightIn - nearH) ) {
          nearW = tempW;
          nearH = tempH;
...
//---------------------------------------------------------------------------------------------------
static bool setSizeAndSubtype(videoDevice * VD, int attemptWidth,
...
int videoInput::start(int deviceID, videoDevice *VD)
{
  ...
  CAPTURE_MODE   		=
    PIN_CATEGORY_CAPTURE; //Don't worry - it ends up being preview (which is faster)
  callbackSetCount 	=
    1;  //make sure callback method is not changed after setup called
  ...
  hr = CoCreateInstance(CLSID_FilterGraph, 0, CLSCTX_INPROC_SERVER,
                        IID_IGraphBuilder, (void**)&VD->pGraph);
  if (FAILED(hr)) {
    if(verbose) {
  ...
  hr = VD->pCaptureGraph->FindInterface(&PIN_CATEGORY_PREVIEW,
                                        &MEDIATYPE_Video, VD->pVideoInputFilter, IID_IAMStreamConfig,
                                        (void **)&streamConfTest);
  if(FAILED(hr)) {
    if(verbose) {
  ...
  if(VD->useCrossbar) {
    ...
    routeCrossbar(&VD->pCaptureGraph, &VD->pVideoInputFilter, VD->connection,
                  CAPTURE_MODE);
  }


  //we do this because webcams don't have a preview mode
  hr = VD->pCaptureGraph->FindInterface(&CAPTURE_MODE, &MEDIATYPE_Video,
                                        VD->pVideoInputFilter, IID_IAMStreamConfig, (void **)&VD->streamConf);
  if(FAILED(hr)) {
    if(verbose) {
  ...
  if(customSize) {
    ...
    if (!foundSize) {
      ...
      for(int i = 0; i < VI_NUM_TYPES; i++) {
        ...
        if(verbose) {
          printf("SETUP: trying format %s @ %i by %i\n", guidStr, VD->tryWidth,
                 VD->tryHeight);
        }
        if( setSizeAndSubtype(VD, VD->tryWidth, VD->tryHeight,
                              mediaSubtypes[i]) ) {
    ...
    if( foundSize == false ) {
      if( verbose ) {
        printf("SETUP: couldn't find requested size - searching for closest matching size\n");
      }
      ...
      findClosestSizeAndSubtype(VD, VD->tryWidth, VD->tryHeight, closestWidth,
                                closestHeight, newMediaSubtype);
  ...
  hr = CoCreateInstance(CLSID_SampleGrabber, NULL, CLSCTX_INPROC_SERVER,
                        IID_IBaseFilter, (void**)&VD->pGrabberF);
  if (FAILED(hr)) {
    if(verbose) {
  ...
  hr = VD->pGrabberF->QueryInterface(IID_ISampleGrabber,
                                     (void**)&VD->pGrabber);
  if (FAILED(hr)) {
    if(verbose) {
  ...
  hr = CoCreateInstance(CLSID_NullRenderer, NULL, CLSCTX_INPROC_SERVER,
                        IID_IBaseFilter, (void**)(&VD->pDestFilter));
  if (FAILED(hr)) {
    if(verbose) {
  ...
  hr = VD->pCaptureGraph->RenderStream(&PIN_CATEGORY_PREVIEW,
                                       &MEDIATYPE_Video, VD->pVideoInputFilter, VD->pGrabberF, VD->pDestFilter);
...
HRESULT videoInput::getDevice(IBaseFilter** gottaFilter, int deviceId,
...
HRESULT videoInput::routeCrossbar(ICaptureGraphBuilder2 **ppBuild,
                                  IBaseFilter **pVidInFilter, int conType, GUID captureMode)
{
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = pBuild->FindInterface(&captureMode, &MEDIATYPE_Interleaved,
                               pVidFilter, IID_IAMCrossbar, (void **)&Crossbar);

    if(hr != NOERROR) {
      hr = pBuild->FindInterface(&captureMode, &MEDIATYPE_Video, pVidFilter,
                                 IID_IAMCrossbar, (void **)&Crossbar);
    }
