...
void pix_mano :: processGrayImage(imageStruct &image)	{
  ...
  if(m_tipData->size < maxtips) {
    delete m_tipData;
    m_tipData=NULL;
  }

  if(!m_tipData)
    m_tipData=new tipData(maxtips);
  ...
  int*t_t=m_tipData->t_t;
  int*v_t=m_tipData->v_t;

  float*tip_coord_x=m_tipData->tip_coord_x;
  float*tip_coord_y=m_tipData->tip_coord_y;
  float*valley_coord_x=m_tipData->valley_coord_x;
  float*valley_coord_y=m_tipData->valley_coord_y;
    
  // index x y magnitude angle state:
  float*t_i=m_tipData->t_i;
  float*t_x=m_tipData->t_x;
  float*t_y=m_tipData->t_y;
  float*t_m=m_tipData->t_m;
  float*t_a=m_tipData->t_a;
  float*t_s=m_tipData->t_s;
  // index x y magnitude angle state:
  float*v_i=m_tipData->v_i;
  float*v_x=m_tipData->v_x;
  float*v_y=m_tipData->v_y;
  float*v_m=m_tipData->v_m;
  float*v_a=m_tipData->v_a;
  float*v_s=m_tipData->v_s;
	
  float*tn_i=m_tipData->tn_i;
  float*tn_x=m_tipData->tn_x;
  float*tn_y=m_tipData->tn_y;
  float*tn_m=m_tipData->tn_m;
  float*tn_a=m_tipData->tn_a;
  float*tn_s=m_tipData->tn_s;
  float*free_i=m_tipData->free_i;
