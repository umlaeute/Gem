...
static RegionReference* new_region( Segmenter *s, int x, int y,
                                    int colour )
{
...
static void remove_adjacent_from( Region *r1,
                                  Region *r2 ) // remove r2 from r1
{
  int i;
#ifndef NDEBUG
  int found = 0;
  ...
  for( i = 0; i < r1->adjacent_region_count; ++i ) {
    if( r1->adjacent_regions[i] == r2 ) {
      r1->adjacent_regions[i] =
        r1->adjacent_regions[ --r1->adjacent_region_count ];
#ifndef NDEBUG
      found = 1;
#endif
      break;
...
static void replace_adjacent( Region *r1, Region *r2,
                              Region *r3 )  // replace r2 with r3 in the adjacency list of r1
{
  int i;
#ifndef NDEBUG
  int found = 0;
...
static void merge_regions( Segmenter *s, Region* r1, Region* r2 )
{
  ...
  assert( r1 != r2 );
  assert( !r1_adjacent_contains_r2( r1, r2 )
          && !r1_adjacent_contains_r2( r2, r1 ) );
  ...
  if( r1->flags & SATURATED_REGION_FLAG ) {
    ...
    for( i = 0; i<r2->adjacent_region_count; ++i ) {
      ...
      if( is_adjacent( a, r1 ) ) {
        ...
        r2->adjacent_regions[i] =
          r2->adjacent_regions[ --r2->adjacent_region_count ];
      }
    }

    if( r1->adjacent_region_count + r2->adjacent_region_count >
        s->max_adjacent_regions ) {
      make_saturated( r1 );
      make_saturated( r2 );
    } else {
      ...
      for( i=0; i < r2->adjacent_region_count; ++i ) {
        ...
        replace_adjacent( a, r2,
                          r1 ); // replace r2 with r1 in the adjacency list of a
...
void initialize_segmenter( Segmenter *s, int width, int height,
                           int max_adjacent_regions )
{
  //max_adjacent_regions += 2; //workaround for #44
  s->max_adjacent_regions = max_adjacent_regions;
  s->region_refs = (RegionReference*)malloc( sizeof(RegionReference) * width
                   * height );
  s->region_ref_count = 0;
  s->sizeof_region = sizeof(Region) + sizeof(Region*) *
                     (max_adjacent_regions-1);
  ...
  s->regions_under_construction = (RegionReference**)malloc( sizeof(
                                    RegionReference*) * width * 2 );
}
...
{
  if( s->region_refs && s->regions
      && s->regions_under_construction /*&& s->spans*/) {
