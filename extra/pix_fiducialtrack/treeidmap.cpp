...
static int find_maximum_tree_depth( const std::string& s )
{
  ...
  for( int i=1; i < (int)s.size();
       ++i ) { // skip first character which is the black/white flag
    ...
    : owner_( treeidmap )
  {
    ...
    if( !is.good() ) {
      ...
      while( is >> s ) {
        ...
        strcpy( ss, s.c_str() );
        std::pair<map_type::iterator, bool> i = treeIdMap_.insert( std::make_pair(
            ss, id++ ) );
        if( i.second ) {
...
{
  return ((TreeIdMapImplementation*)
          treeidmap->implementation_)->treestring_to_id( treestring );
}
