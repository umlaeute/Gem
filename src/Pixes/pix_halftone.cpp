...
void pix_halftone :: processRGBAImage(imageStruct &image)
{
    ...
    int nCellSize=static_cast<int>(m_CellSize);
    nCellSize=GateInt(nCellSize,1,nMaxCellSize);

    int nStyle=static_cast<int>(m_Style);
    nStyle=GateInt(nStyle,0,4);

    int nSmoothingThreshold=static_cast<int>(m_Smoothing);
...
void pix_halftone :: Pete_HalfTone_CalcCorners(int nWidth,int nHeight,float AngleRadians,int nCellSize,
