////////////////////////////////////////////////////////
//
// GEM - Graphics Environment for Multimedia
//
// zmoelnig@iem.kug.ac.at
//
// Implementation file
//
//    Copyright (c) 1997-2000 Mark Danks.
//    Copyright (c) G¸nther Geiger.
//    Copyright (c) 2001-2002 IOhannes m zmoelnig. forum::f¸r::uml‰ute. IEM
//    For information on usage and redistribution, and for a DISCLAIMER OF ALL
//    WARRANTIES, see the file, "GEM.LICENSE.TERMS" in this distribution.
//
/////////////////////////////////////////////////////////

#include "vertex_sub.h"

CPPEXTERN_NEW_WITH_GIMME(vertex_sub)
 
/////////////////////////////////////////////////////////
//
// vertex_sub
//
/////////////////////////////////////////////////////////
// Constructor
//
/////////////////////////////////////////////////////////
vertex_sub :: vertex_sub(int argc, t_atom*argv) : vertex_add(argc, argv)
{}

/////////////////////////////////////////////////////////
// Destructor
//
/////////////////////////////////////////////////////////
vertex_sub :: ~vertex_sub()
{}


/////////////////////////////////////////////////////////
// render
//
/////////////////////////////////////////////////////////
// we assume that "lsize" and "rsize" are >0
// we assume that "larray" and "larray" point somewhere
// checking is done in render()
void vertex_sub :: vertexProcess(int lsize, float*larray, int rsize, float*rarray){
  float indR=0.f; // the right-hand index
  float incR=(float)rsize/(float)lsize; // the right-hand increment

  for(int i=0; i<lsize; i++){
    const int I=4*i;
    const int J=4*(int)indR; // i know that this is expensive
    larray[I+0]-=rarray[J+0];
    larray[I+1]-=rarray[J+1];
    larray[I+2]-=rarray[J+2];
    larray[I+3]-=rarray[J+3];
    indR+=incR;
  }
}

 
/////////////////////////////////////////////////////////
// static member function
//
/////////////////////////////////////////////////////////
void vertex_sub :: obj_setupCallback(t_class *classPtr)
{}
