...
REGISTER_VIDEOFACTORY("v4l2", videoUNICAP);

videoUNICAP :: videoUNICAP() : video("v4l2", 0)
                  ...
                  void *                 arg)
{
  ...
  debugIOCTL("UNICAP: xioctl %d\n", req);
  do {
    r = v4l2_ioctl (fd, request, arg);
    debugIOCTL("UNICAP: xioctl %d->%d\n", r, errno);
  }
  while (-1 == r && EINTR == errno);

  debugIOCTL("UNICAP: xioctl done %d\n", r);
#if 0
  if(r!=0) {
    char buf[13];
...
static int reqbufs(int fd, unsigned int numbufs) {
  ...
  req.count               = numbufs;
  req.type                = UNICAP_BUF_TYPE_VIDEO_CAPTURE;
  req.memory              = UNICAP_MEMORY_MMAP;
...
int videoUNICAP::init_mmap (void)
{
  const char *devname=(m_devicename.empty())?"device":m_devicename.c_str();
  int count = reqbufs(m_tvfd, UNICAP_NBUF);
  ...
  for (m_nbuffers = 0; m_nbuffers < count; ++m_nbuffers) {
    ...
    buf.type        = UNICAP_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory      = UNICAP_MEMORY_MMAP;
...
//
/////////////////////////////////////////////////////////
void *videoUNICAP::capturing_(void*you)
{
  videoUNICAP *me=(videoUNICAP *)you;
  return me->capturing();
}
void *videoUNICAP :: capturing(void)
{
  int errorcount=0;
  ...
  while(m_continue_thread){
    ...
    debugThread("UNICAP: grabbed %d", buf.index);
  ...
  // stop capturing
  m_capturing=false;
  debugThread("UNICAP: thread finished");
  return NULL;
}
...
#endif
      case UNICAP_PIX_FMT_RGB24: CONVERT(RGB   ); break;
      case UNICAP_PIX_FMT_BGR32: CONVERT(BGRA  ); break;
      case UNICAP_PIX_FMT_RGB32: CONVERT(ARGB  ); break;
      case UNICAP_PIX_FMT_GREY : CONVERT(Gray  ); break;
      case UNICAP_PIX_FMT_UYVY : CONVERT(YUV422); break;
      case UNICAP_PIX_FMT_YUYV : CONVERT(YUY2  ); break;
      case UNICAP_PIX_FMT_YUV420:CONVERT(YU12  ); break;
...
bool videoUNICAP :: openDevice(gem::Properties&props) {
  ...
  if (-1 == xioctl (m_tvfd, VIDIOC_QUERYCAP, &cap)) {
    if (EINVAL == errno) {
      error("%s is no UNICAP device",  dev_name);
      closeDevice(); return false;
    } else {
  ...
  if (!(cap.capabilities & UNICAP_CAP_VIDEO_CAPTURE)) {
    error("%s is no video capture device", dev_name);
    closeDevice(); return false;
  }

  if (!(cap.capabilities & UNICAP_CAP_STREAMING)) {
...
//
/////////////////////////////////////////////////////////
bool videoUNICAP :: restartTransfer()
{
  bool rendering=m_rendering;
...
bool videoUNICAP :: startTransfer()
{
  ...
  fmt.type                = UNICAP_BUF_TYPE_VIDEO_CAPTURE;
  ...
  case GL_YCBCR_422_GEM: 
    pixelformat = UNICAP_PIX_FMT_UYVY; 
    break;
  case GL_LUMINANCE: 
    pixelformat = UNICAP_PIX_FMT_GREY; 
    break;
  case GL_RGB: 
    pixelformat = UNICAP_PIX_FMT_RGB24; 
    break;
  default: 
    pixelformat = UNICAP_PIX_FMT_RGB32; 
    m_reqFormat=GL_RGBA;
    break;
  }
  ...
  switch(m_gotFormat){
  case UNICAP_PIX_FMT_RGB32: debugPost("v4l2: ARGB");break;
  case UNICAP_PIX_FMT_RGB24: debugPost("v4l2: RGB");break;
  case UNICAP_PIX_FMT_UYVY:  debugPost("v4l2: YUV ");break;
  case UNICAP_PIX_FMT_GREY:  debugPost("v4l2: gray");break;
  case UNICAP_PIX_FMT_YUV420:debugPost("v4l2: YUV 4:2:0");break;
  default: 
    ...
    case GL_YCBCR_422_GEM: 
    case GL_LUMINANCE: 
      pixelformat = UNICAP_PIX_FMT_YUV420; 
      break;
    default: 
      pixelformat = UNICAP_PIX_FMT_RGB24; 
      break;
  ...
  switch(m_gotFormat){
  case UNICAP_PIX_FMT_RGB32: case UNICAP_PIX_FMT_RGB24:
  case UNICAP_PIX_FMT_UYVY: case UNICAP_PIX_FMT_YUV420: case UNICAP_PIX_FMT_YUYV: 
  case UNICAP_PIX_FMT_GREY: 
    break;
  default: 
  ...
  type = UNICAP_BUF_TYPE_VIDEO_CAPTURE;
  ...
  debugPost("v4l2: format: %c%c%c%c -> 0x%X", 
            ...
            m_reqFormat);
  switch(m_gotFormat){
  case UNICAP_PIX_FMT_GREY  : m_colorConvert=(m_reqFormat!=GL_LUMINANCE); break;
  case UNICAP_PIX_FMT_RGB24 : m_colorConvert=(m_reqFormat!=GL_BGR); break;
  case UNICAP_PIX_FMT_RGB32 : m_colorConvert=true; break;
  case UNICAP_PIX_FMT_UYVY  : m_colorConvert=(m_reqFormat!=GL_YCBCR_422_GEM); break;
  case UNICAP_PIX_FMT_YUV420: m_colorConvert=1; break;
...
std::vector<std::string> videoUNICAP::enumerate() {
  ...
  for(i=0; i<allglob.size(); i++) {
    ...
    verbose(2, "UNICAP: found possible device %s", dev.c_str());
    int fd=v4l2_open(dev.c_str(), O_RDWR);
    verbose(2, "UNICAP: v4l2_open returned %d", fd);
    if(fd<0)continue;
    struct v4l2_capability cap;
...
void videoUNICAP::addProperties(struct v4l2_queryctrl queryctrl,
			  gem::Properties&readable,
			  gem::Properties&writeable) {
  const char* name=NULL;
  gem::any typ;

  if (queryctrl.flags & UNICAP_CTRL_FLAG_DISABLED)
    return;

  switch(queryctrl.type) {
  case UNICAP_CTRL_TYPE_BUTTON:
    break;
  case UNICAP_CTRL_TYPE_BOOLEAN:
    typ=1;
    break;
  case UNICAP_CTRL_TYPE_MENU:
    typ=queryctrl.maximum;
    break;
  case UNICAP_CTRL_TYPE_INTEGER:
    typ=queryctrl.maximum;
    break;
  case UNICAP_CTRL_TYPE_INTEGER64:
    typ=0;
    break;
  default:
    return;
  ...
  if (!(queryctrl.flags & UNICAP_CTRL_FLAG_READ_ONLY)) {
...
bool videoUNICAP :: enumProperties(gem::Properties&readable,
  ...
  for (id = UNICAP_CID_BASE;
       id < UNICAP_CID_LASTP1;
       id++) {
    queryctrl.id = id;
  ...
  for (id = UNICAP_CID_PRIVATE_BASE;;
       id++) {
    queryctrl.id = id;
    if (0 == xioctl (m_tvfd, VIDIOC_QUERYCTRL, &queryctrl)) {
      addProperties(queryctrl, readable, writeable);
...
void videoUNICAP :: getProperties(gem::Properties&props) {
  ...
  for(i=0; i<keys.size(); i++) {
	...
	if(0==xioctl(m_tvfd, VIDIOC_G_STD, &stdid)) {
	  ...
	  switch(stdid) {
	  default:
	  case UNICAP_STD_UNKNOWN: std="UNKNOWN"; break; 
	  case UNICAP_STD_ALL: std="ALL"; break; 

	  case UNICAP_STD_ATSC: std="ATSC"; break; 
	  case UNICAP_STD_625_50: std="625_50"; break; 
	  case UNICAP_STD_525_60: std="525_60"; break; 
	  case UNICAP_STD_SECAM: std="SECAM"; break; 
	  case UNICAP_STD_SECAM_DK: std="SECAM_DK"; break; 
	  case UNICAP_STD_NTSC: std="NTSC"; break; 
	  case UNICAP_STD_PAL: std="PAL"; break; 
	  case UNICAP_STD_PAL_DK: std="PAL_DK"; break; 
	  case UNICAP_STD_PAL_BG: std="PAL_BG"; break; 
	  case UNICAP_STD_DK: std="DK"; break; 
	  case UNICAP_STD_GH: std="GH"; break; 
	  case UNICAP_STD_B: std="B"; break; 
	  case UNICAP_STD_MN: std="MN"; break; 
	  case UNICAP_STD_ATSC_16_VSB: std="ATSC_16_VSB"; break; 
	  case UNICAP_STD_ATSC_8_VSB: std="ATSC_8_VSB"; break; 
	  case UNICAP_STD_SECAM_LC: std="SECAM_LC"; break; 
	  case UNICAP_STD_SECAM_L: std="SECAM_L"; break; 
	  case UNICAP_STD_SECAM_K1: std="SECAM_K1"; break; 
	  case UNICAP_STD_SECAM_K: std="SECAM_K"; break; 
	  case UNICAP_STD_SECAM_H: std="SECAM_H"; break; 
	  case UNICAP_STD_SECAM_G: std="SECAM_G"; break; 
	  case UNICAP_STD_SECAM_D: std="SECAM_D"; break; 
	  case UNICAP_STD_SECAM_B: std="SECAM_B"; break; 
	  case UNICAP_STD_NTSC_M_KR: std="NTSC_M_KR"; break; 
	  case UNICAP_STD_NTSC_443: std="NTSC_443"; break; 
	  case UNICAP_STD_NTSC_M_JP: std="NTSC_M_JP"; break; 
	  case UNICAP_STD_NTSC_M: std="NTSC_M"; break; 
	  case UNICAP_STD_PAL_60: std="PAL_60"; break; 
	  case UNICAP_STD_PAL_Nc: std="PAL_Nc"; break; 
	  case UNICAP_STD_PAL_N: std="PAL_N"; break; 
	  case UNICAP_STD_PAL_M: std="PAL_M"; break; 
	  case UNICAP_STD_PAL_K: std="PAL_K"; break; 
	  case UNICAP_STD_PAL_D1: std="PAL_D1"; break; 
	  case UNICAP_STD_PAL_D: std="PAL_D"; break; 
	  case UNICAP_STD_PAL_I: std="PAL_I"; break; 
	  case UNICAP_STD_PAL_H: std="PAL_H"; break; 
	  case UNICAP_STD_PAL_G: std="PAL_G"; break; 
	  case UNICAP_STD_PAL_B1: std="PAL_B1"; break; 
	  case UNICAP_STD_PAL_B: std="PAL_B"; break; 
	  }
	  props.set("norm", std);
	}
      } else if("channel" == key) {
...
void videoUNICAP :: setProperties(gem::Properties&props) {
  ...
  for(i=0; i<keys.size(); i++) {
	...
	vc.value=d;
      ...
      if(UNICAP_CTRL_TYPE_BUTTON == qc.type) {
	props.erase(key);
      }
    } else {
      if("norm" == key) {
	v4l2_std_id stdid = UNICAP_STD_UNKNOWN;
	std::string std;
	switch(props.type(key)) {
	  case gem::Properties::STRING:
	    if(props.get(key, std)) {
	      if("ALL" == std)  stdid=UNICAP_STD_ALL;
	      else if("PAL_B" == std)  stdid=UNICAP_STD_PAL_B;
	      else if("PAL_B1" == std)  stdid=UNICAP_STD_PAL_B1;
	      else if("PAL_G" == std)  stdid=UNICAP_STD_PAL_G;
	      else if("PAL_H" == std)  stdid=UNICAP_STD_PAL_H;
	      else if("PAL_I" == std)  stdid=UNICAP_STD_PAL_I;
	      else if("PAL_D" == std)  stdid=UNICAP_STD_PAL_D;
	      else if("PAL_D1" == std)  stdid=UNICAP_STD_PAL_D1;
	      else if("PAL_K" == std)  stdid=UNICAP_STD_PAL_K;
	      else if("PAL_M" == std)  stdid=UNICAP_STD_PAL_M;
	      else if("PAL_N" == std)  stdid=UNICAP_STD_PAL_N;
	      else if("PAL_Nc" == std)  stdid=UNICAP_STD_PAL_Nc;
	      else if("PAL_60" == std)  stdid=UNICAP_STD_PAL_60;
	      else if("NTSC_M" == std)  stdid=UNICAP_STD_NTSC_M;
	      else if("NTSC_M_JP" == std)  stdid=UNICAP_STD_NTSC_M_JP;
	      else if("NTSC_443" == std)  stdid=UNICAP_STD_NTSC_443;
	      else if("NTSC_M_KR" == std)  stdid=UNICAP_STD_NTSC_M_KR;
	      else if("SECAM_B" == std)  stdid=UNICAP_STD_SECAM_B;
	      else if("SECAM_D" == std)  stdid=UNICAP_STD_SECAM_D;
	      else if("SECAM_G" == std)  stdid=UNICAP_STD_SECAM_G;
	      else if("SECAM_H" == std)  stdid=UNICAP_STD_SECAM_H;
	      else if("SECAM_K" == std)  stdid=UNICAP_STD_SECAM_K;
	      else if("SECAM_K1" == std)  stdid=UNICAP_STD_SECAM_K1;
	      else if("SECAM_L" == std)  stdid=UNICAP_STD_SECAM_L;
	      else if("SECAM_LC" == std)  stdid=UNICAP_STD_SECAM_LC;
	      else if("ATSC_8_VSB" == std)  stdid=UNICAP_STD_ATSC_8_VSB;
	      else if("ATSC_16_VSB" == std)  stdid=UNICAP_STD_ATSC_16_VSB;
	      else if("MN" == std)  stdid=UNICAP_STD_MN;
	      else if("B" == std)  stdid=UNICAP_STD_B;
	      else if("GH" == std)  stdid=UNICAP_STD_GH;
	      else if("DK" == std)  stdid=UNICAP_STD_DK;
	      else if("PAL_BG" == std)  stdid=UNICAP_STD_PAL_BG;
	      else if("PAL_DK" == std)  stdid=UNICAP_STD_PAL_DK;
	      else if("PAL" == std)  stdid=UNICAP_STD_PAL;
	      else if("NTSC" == std)  stdid=UNICAP_STD_NTSC;
	      else if("SECAM_DK" == std)  stdid=UNICAP_STD_SECAM_DK;
	      else if("SECAM" == std)  stdid=UNICAP_STD_SECAM;
	      else if("525_60" == std)  stdid=UNICAP_STD_525_60;
	      else if("625_50" == std)  stdid=UNICAP_STD_625_50;
	      else if("ATSC" == std)  stdid=UNICAP_STD_ATSC;
	    }
	    break;
	default:
	  continue;
  ...
  if(restart) {
    ...
    if(setcropping) {
      ...
      cropcap.type = UNICAP_BUF_TYPE_VIDEO_CAPTURE;
	...
	crop.type = UNICAP_BUF_TYPE_VIDEO_CAPTURE;
