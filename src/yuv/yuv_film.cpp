...
CPPEXTERN_NEW_WITH_ONE_ARG(yuv_film, t_symbol *, A_DEFSYM)
...
//
/////////////////////////////////////////////////////////
yuv_film :: yuv_film(t_symbol *filename) :
...
//  memset(m_data, 0, dataSize);
  ...
  post("yuv_film: created buffer @ %x", m_data);
}
...
void yuv_film :: openMess(t_symbol *filename)
{
  ...
  post("GEM: yuv_film: Loaded file: %s with %d frames (%dx%d)", buf, m_numFrames, m_xsize, m_ysize);
}
...
//
/////////////////////////////////////////////////////////
void yuv_film :: startRendering()
{
  m_pixBlock.newimage = 1;
}
void yuv_film :: render(GemState *state)
{
...
//
/////////////////////////////////////////////////////////
void yuv_film :: postrender(GemState *state)
{
...
//
/////////////////////////////////////////////////////////
void yuv_film :: changeImage(int imgNum, int trackNum)
{
  if (imgNum < 0){
    error("GEM: yuv_film: selection number must be > 0");
    imgNum=0;
  }
  ...
  case GEM_MOVIE_MOV:
#ifdef DEBUG
    post("yuv_film: trackNum = %d", trackNum);
#endif
    if (trackNum < 0)trackNum=0;
    if (trackNum > m_numTracks-1) error("GEM: yuv_film: track %d number too high (max %d) ", trackNum, m_numTracks-1);
  ...
  default:
    if (imgNum > m_numFrames) {
      ...
      //      else error("GEM: yuv_film: frame %d exceeds max (%d)", imgNum, m_numFrames);
      //m_reqFrame = imgNum;
      return;
...
//
/////////////////////////////////////////////////////////
void yuv_film :: obj_setupCallback(t_class *classPtr)
{}

void yuv_film :: openMessCallback(void *data, t_symbol *filename)
{
    GetMyClass(data)->openMess(filename);
}

void yuv_film :: changeImageCallback(void *data, t_symbol *, int argc, t_atom *argv)
{
    GetMyClass(data)->changeImage((argc<1)?0:atom_getint(argv), (argc<2)?0:atom_getint(argv+1));
}

void yuv_film :: autoCallback(void *data, t_floatarg state)
{
