...
uniform sampler2DRect texture_bruit, texture_mass, texture_mass_old, texture_normal;
uniform float init, gravite;
uniform vec2 wind;
uniform float K,D; // retour au point d'equilibre
...
void main (void)
{
	...
	if ( (coord.x < 92.) && (coord.y <  64.) )
	{
		...
		force += -K*(pos-vec4(0.5)-vec4(coord.x/1000.,(coord.y-64.)/1000.,0.,0.)); 
			// retour au point d'equilibre
			// non utilisé normallement	

	// 4 liens direct (gauche / droite et haut / bas)
		// on ajoute une force ssi la taille est > epsilon (en dessous le calcul de la normal de la force est incertain
		// TODO tester avec epsilon = 0 car le test est strict
		// on limite aussi ds l'espace pour ne prendre en compte que le lien valide (effet de bord)

		dist = pos.xyz - posG.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x >  1.) )
			{ force.xyz += -K1 * (taille - 1./1000.)* normalize(dist); }

		dist = pos.xyz - posD.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x < 91.) )
			{ force.xyz += -K1 * (taille - 1./1000.)* normalize(dist); }

		dist = pos.xyz - posH.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.y < 63.) )
			{ force.xyz += -K1 * (taille - 1./1000.)* normalize(dist); }

		dist = pos.xyz - posB.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.y >  1.) )
			{ force.xyz += -K1 * (taille - 1./1000.)* normalize(dist); }

	// 4 liens diagonaux (haut gauche, bas droite, etc)
		dist = pos.xyz - posHD.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x < 91.) && (coord.y >  1.) )
			{ force.xyz += -K1 * (taille - 1.4142/1000.)* normalize(dist); }
	 
		dist = pos.xyz - posBG.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x >  1.) && (coord.y < 63.) )
			{ force.xyz += -K1 * (taille - 1.4142/1000.)* normalize(dist); }

		dist = pos.xyz - posHG.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x < 91.) && (coord.y < 63.) )
			{ force.xyz += -K1 * (taille - 1.4142/1000.)* normalize(dist); }

		dist = pos.xyz - posBD.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x >  1.) && (coord.y >  1.) )
			{ force.xyz += -K1 * (taille - 1.4142/1000.)* normalize(dist); }

	// 4 liens double longeur (rigidité de flexion)
		dist = pos.xyz - pos2G.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x >  2.) )
			{ force.xyz += -K1 * (taille - 2./1000.)* normalize(dist); }

		dist = pos.xyz - pos2D.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.x < 90.) )
			{ force.xyz += -K1 * (taille - 2./1000.)* normalize(dist); }

		dist = pos.xyz - pos2H.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.y < 62.) )
			{ force.xyz += -K1 * (taille - 2./1000.)* normalize(dist); }

		dist = pos.xyz - pos2B.xyz ; 
		taille = length(dist) ;
		if ( (taille > 0.0000001) && (coord.y >  2.) )
			{ force.xyz += -K1 * (taille - 2./1000.)* normalize(dist); }
	...
	// ajout d'un pseuo buit
		float W_noiseX = cos(t + 0.00002353 * gl_TexCoord[0].t + 0.0000234434* gl_TexCoord[0].s + 345.2342);
	...
	// preparation de la sortie
		...
		force += vec4(0.5,0.5,0.5,1.);
			// ajout de l'ofset pour etre entre -1 et 1
		force.a = 1.;
			// on vire la 4eme composante au cas ou.
	}

	gl_FragColor = mix(force,vec4(0.5,0.5,0.5,1.),init);
//	gl_FragColor = vec4(W_noiseX/2.+0.5,W_noiseY/2.+0.5,W_noiseZ/2.+0.5,1.);
//	gl_FragColor = vec4(W_noiseY/2.+0.5,0.,0.,1.);
